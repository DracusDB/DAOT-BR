...
public class WarhammerSyringeItem extends Item {
    public WarhammerSyringeItem(Settings settings) {
        super(settings);
    }
    ...
    public void appendTooltip (ItemStack itemStack, TooltipContext context, List<Text> tooltip, TooltipType type) {
        tooltip.add((Text.literal("It controls Titan hardening to create weapons, or anything it needs, from underground.").formatted(Formatting.GRAY, Formatting.ITALIC)));
    }
        ...
        if (!world.isClient() && user instanceof ServerPlayerEntity serverPlayer) {
            ...
            if (server != null) {
                ...
                for (ServerPlayerEntity p : server.getPlayerManager().getPlayerList()) {
                    p.sendMessage(
                            Text.literal(playerName + " has claimed the power of the Warhammer Titan!")
