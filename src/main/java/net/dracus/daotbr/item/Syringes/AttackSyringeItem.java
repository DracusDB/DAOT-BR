...
public class AttackSyringeItem extends Item {
    public AttackSyringeItem(Settings settings) {
        super(settings);
    }
    ...
    public void appendTooltip (ItemStack itemStack, Item.TooltipContext context, List<Text> tooltip, TooltipType type) {
        tooltip.add((Text.literal("No matter which era this Titan has found itself in, it has pushed ever forward,").formatted(Formatting.AQUA, Formatting.ITALIC)));
        tooltip.add((Text.literal("seeking out freedom. For the sake of freedom, it fights.").formatted(Formatting.AQUA, Formatting.ITALIC)));
    }
        ...
        if (!world.isClient() && user instanceof ServerPlayerEntity serverPlayer) {
            ...
            if (server != null) {
                ...
                for (ServerPlayerEntity p : server.getPlayerManager().getPlayerList()) {
                    p.sendMessage(
                            Text.literal(playerName + " has claimed the power of the Attack Titan!")
